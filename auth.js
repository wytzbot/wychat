...
export async function sendMagicLink(email) {
  const res=await withRetry(async()=>{
    let r;
    try{
      r=await fetch("/api/send-signin-link",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({email})
      });
    }catch(networkErr){
      // fetch() itself throwing (not just a non-2xx response) means the
      // request never reached the server — treat it the same as Firebase's
      // network-request-failed so the retry loop above catches it too.
      const err=new Error("Network request failed.");
      err.code="auth/network-request-failed";
      throw err;
    }
    const data=await r.json().catch(()=>({}));
    if(!r.ok || !data.ok) throw new Error(data.error||"Couldn't send the sign-in email.");
    return data;
  });
...
// quota, no SMTP account, and no per-send cost — the most reliable fallback
// when email delivery is being finicky, and often the faster choice anyway.

...
/**
 * Counts messages for a room since a given date.
 * Used for the Starter 10,000-message monthly allowance.
 */
export async function countMessagesSince(roomId, sinceDate) {
  const q = query(
    collection(db, "messages"),
    where("roomId", "==", roomId)
  );
  ...
  for (const d of snap.docs) {
    ...
    if (ts && ts >= sinceDate) count++;
  }
  return count;
}


export async function getRoomMessageStats(roomId, sinceDate = null) {
  ...
  for (const d of snap.docs) {
    const data = d.data();
    total++;
    if (data.participantId) participants.add(data.participantId);
    const ts = data.createdAt?.toDate?.();
    if (!sinceDate || (ts && ts >= sinceDate)) thisPeriod++;
  }
  return { total, thisPeriod, uniqueParticipants: participants.size };
}

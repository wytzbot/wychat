...
import { watchAuth, sendMagicLink, finishMagicLink, logout } from "./auth.js";
import { route } from "./router.js";
import { getRoom, createRoom, ownerRooms, setRoomStatus, removeRoom, randomId } from "./rooms.js";
import { subscribeMessages, sendMessage, editMessage, deleteMessage } from "./messages.js";
...
async function loadPlan(){
  try{
    ...
    state.plan={plan:s.plan,ads:s.ads,ready:true};
  }catch{
    state.plan={plan:"free",ads:true,ready:true};
  }
}
...
function home(){
  ...
  <section class="card"><h2>One link. One shared conversation.</h2><p>No account for participants. No profiles. No @mentions. Just a room-specific anonymous ID, live replies, quotes, and a 30-day lifespan.</p></section>`);
...
async function dashboard(){
  ...
  const atLimit=isFree && rooms.length>=FREE_ROOM_LIMIT;
  const slugField = isFree
  ...
  ${atLimit?`<section class="card upgrade-banner"><h2>You\u2019ve reached the free plan\u2019s ${FREE_ROOM_LIMIT}-room limit</h2><p>Close or delete an old room, or upgrade for unlimited rooms, a custom link, and no ads.</p><a class="primary" href="/pricing" data-navlink>See plans</a></section>`:`
  ...
  document.querySelector("#newroom")?.addEventListener("click",async()=>{
    ...
    if(!q)return;
    try{const id=await createRoom(state.user.uid,q,slug);location.href="/q/"+id;}
...
function roomCard(r){return `<article class="roomcard"><span class="status">${r.status==="live"?"LIVE":"CLOSED"}</span><h3>${esc(r.question)}</h3><p>Room link: /q/${esc(r.slug||r.roomId)}</p><div class="roomactions"><button data-open="${esc(r.slug||r.roomId)}">Open</button><button data-copy="${esc(r.slug||r.roomId)}">Copy</button>${r.status==="live"?`<button data-close="${r.roomId}">Close</button>`:`<button data-reopen="${r.roomId}">Reopen</button>`}<button data-delete="${r.roomId}">Delete</button></div></article>`}
...
async function roomPage(key){
  ...
  shell(`<section class="roomtop"><a href="/" class="back" data-navlink>\u2039</a><div><span class="eyebrow">ANONYMOUS ROOM</span><h1>${esc(room.question)}</h1><span class="live">${room.status==="live"?"\u25CF LIVE":"CLOSED — read only"}</span></div></section>
  ...
  ${room.status==="live"?`<form id="composer" class="composer"><button type="button" id="plus" class="plus">+</button><input id="message" maxlength="2000" placeholder="Type anonymously…" autocomplete="off"><button class="send" type="submit">Send</button></form>`:`<div class="closednote">This room is closed. You can still read the conversation.</div>`}
  ...
  state.unsubscribe=subscribeMessages(room.roomId,(changes,_,err)=>{
    ...
    changes?.forEach(ch=>{if(ch.type==="removed")state.messages.delete(ch.doc.id);else state.messages.set(ch.doc.id,{messageId:ch.doc.id,...ch.doc.data()});});
    renderMessages();
  });
  ...
  function renderMessages(){
    ...
    conv.innerHTML=vals.map(m=>`<article class="msg" id="m-${m.messageId}"><div class="bubble"><b>${esc(m.participantId)}</b>${m.quotedMessageId?`<div class="quoted">\u21B3 ${esc(m.quoteSnapshot?.participantId)} "${esc(m.quoteSnapshot?.content)}"</div>`:""}<p>${esc(m.content)}</p><div class="meta">${m.edited?"edited · ":""}${m.createdAt?.toDate?m.createdAt.toDate().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):"sending"} · <button data-quote="${m.messageId}">Quote</button> <button data-report="${m.messageId}">Report</button>${m.participantId===state.participant?` <button data-edit="${m.messageId}">Edit</button> <button data-del="${m.messageId}">Delete</button>`:""}${isOwner&&m.participantId!==state.participant?` <button class="block-btn" data-block="${esc(m.participantId)}">Block</button>`:""}</div></div></article>`).join("")||`<div class="empty"><h2>No opinions yet \u{1F440}</h2><small>Share your link and let the conversation begin.</small></div>`;
  ...
  const retentionDays = state.plan.plan==="free" ? 30 : 90;
...
function simplePage(name){
  const pages={
    pricing:["Plans","Free — ₦0","10 question rooms · anonymous conversation · quotes · moderation · banner ads","Starter — ₦900/month","Unlimited rooms · no ads","Pro — ₦1,500/month","Unlimited rooms · custom links · 90-day message retention · no ads"],
    "how-it-works":["How WyChat works","ASK → SHARE → TALK → DISAPPEAR","Create a question, share the room link, and let people join without an account. Every participant gets a random room-specific ID. Messages expire individually after 30 days (90 days on paid plans)."],
  ...
  if(name==="pricing"){
    document.querySelector(".legal").insertAdjacentHTML("afterend",`<div class="region-switch"><button class="secondary" id="ngPrices">Nigeria · ₦</button><button class="secondary" id="intlPrices">International · $</button></div><div class="plans" id="plans"></div>`);
    const renderPrices=region=>{
      const intl=region==="intl";
      document.querySelector("#plans").innerHTML=
        `<button class="plan" data-plan="starter" data-region="${region}"><b>Starter</b><strong>${intl?"$1":"₦900"}/month</strong><span>Unlimited rooms · No ads</span></button>
         <button class="plan" data-plan="pro" data-region="${region}"><b>Pro</b><strong>${intl?"$2":"₦1,500"}/month</strong><span>Unlimited rooms · Custom links · 90-day retention · No ads</span></button>`;
...
async function start(){
  ...
  try{await finishMagicLink(promptForEmail);}catch(e){console.warn(e);toast(e.message||"Couldn't finish signing you in.","error");}
  watchAuth(async u=>{
    state.user=u;
    if(u){ await loadPlan(); await handlePaymentReturn(); await resumePendingPlan(); }
    else { state.plan={plan:"free",ads:true,ready:true}; }
    const r=route();

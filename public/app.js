...
import { watchAuth, sendMagicLink, finishMagicLink, logout, signInWithGoogle } from "./auth.js";
...
function signin(){
  ...
  <input id="email" type="email" autocomplete="email" placeholder="you@example.com"><button class="primary" id="magic">Send secure sign-in link</button><small id="authmsg"></small><div id="freeAd" class="free-ad"><div class="ad-label">ADVERTISEMENT</div><div id="ad-slot"></div></div></section>`,{menu:true});
  ...
  document.querySelector("#googleSignin").onclick=async()=>{
    ...
    catch(e){ if(e.code!=="auth/popup-closed-by-user") toast(e.message||"Couldn't sign in with Google.","error"); }
  };
...
async function dashboard(){
  ...
  document.querySelector("#newroom")?.addEventListener("click",async()=>{
    ...
    if(!q)return;
    try{const id=await createRoom(state.user.uid,q,slug,isFree?30:90);location.href="/q/"+id;}
...
async function roomPage(key){
  ...
  const retentionDays = room.retentionDays ?? 30;
  const roomCreatedAtMs = room.createdAt?.toMillis?.() ?? Date.now();
  const clearsOn=new Date(roomCreatedAtMs+retentionDays*86400000).toLocaleDateString(undefined,{month:"short",day:"numeric",year:"numeric"});
  shell(`<section class="roomtop"><a href="/" class="back" data-navlink>\u2039</a><div><span class="eyebrow">ANONYMOUS ROOM</span><h1>${esc(room.question)}</h1><span class="live">${room.status==="live"?"\u25CF LIVE":"CLOSED — read only"}</span><small class="retention-note">Messages stay here for everyone until ${clearsOn}, then they're cleared automatically.</small></div></section>

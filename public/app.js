...
import { watchAuth, logout, signInWithGoogle, finishGoogleRedirect } from "./auth.js";
import { route } from "./router.js";
import { getRoom, createRoom, ownerRooms, setRoomStatus, removeRoom, updateRoom, randomId } from "./rooms.js";
import { subscribeMessages, sendMessage, editMessage, deleteMessage, countMessagesSince, getRoomMessageStats } from "./messages.js";
...
function signin(){
  ...
  <small id="authmsg" class="auth-hint">Having trouble signing in? It's often a network/carrier issue — try switching to Wi-Fi or another network provider (e.g. MTN, Airtel, Glo) and try again.</small>
  ...
  document.querySelector("#googleSignin").onclick=async()=>{
    ...
    if(msg) msg.textContent="Trying Google Sign-In. If the popup is blocked or the network drops, WyChat will automatically switch to the more reliable redirect sign-in.";
    try{
      await signInWithGoogle();
    }catch(e){
      btn.disabled=false;
      btn.innerHTML=`<svg width="18" height="18" viewBox="0 0 18 18"><path fill="#4285F4" d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.9c1.7-1.57 2.7-3.88 2.7-6.62z"/><path fill="#34A853" d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.9-2.26c-.8.54-1.84.86-3.06.86-2.35 0-4.34-1.59-5.05-3.72H.98v2.33A9 9 0 0 0 9 18z"/><path fill="#FBBC05" d="M3.95 10.7A5.4 5.4 0 0 1 3.67 9c0-.59.1-1.17.28-1.7V4.97H.98A9 9 0 0 0 .0 9c0 1.45.35 2.83.98 4.03l2.97-2.33z"/><path fill="#EA4335" d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0A9 9 0 0 0 .98 4.97l2.97 2.33C4.66 5.17 6.65 3.58 9 3.58z"/></svg> Continue with Google`;
      const code=e?.code||"unknown";
      const network=code==="auth/network-request-failed";
      if(msg) msg.textContent=network
        ? "Google could not be reached right now. WyChat is still working — tap Continue with Google again, or try another connection."
...
function roomCard(r){return `<article class="roomcard"><span class="status">${r.status==="live"?"LIVE":"CLOSED"}</span><h3>${esc(r.question)}</h3><p>Room link: /q/${esc(r.slug||r.roomId)}</p><div class="roomactions"><button data-open="${esc(r.slug||r.roomId)}">Open</button><button data-analytics="${r.roomId}">Analytics</button><button data-copy="${esc(r.slug||r.roomId)}">Copy</button>${r.status==="live"?`<button data-close="${r.roomId}">Close</button>`:`<button data-reopen="${r.roomId}">Reopen</button>`}<button data-delete="${r.roomId}">Delete</button></div></article>`}
...
async function roomPage(key){
  ...
  if(roomPlan==="starter"){
    try{
      const start=new Date(); start.setDate(1); start.setHours(0,0,0,0);
      monthlyMessageCount=await countMessagesSince, getRoomMessageStats(room.roomId,start);
  ...
  function renderMessages(){
    ...
    for(const m of vals){
      ...
      html+=`<article class="msg" id="m-${m.messageId}"><div class="bubble${m.unconfirmed?" unconfirmed":""}"><b>${esc(m.participantId)}</b>${m.quotedMessageId?`<div class="quoted">\u21B3 ${esc(m.quoteSnapshot?.participantId)} "${esc(m.quoteSnapshot?.content)}"</div>`:""}<p>${esc(m.content)}</p><div class="meta">${m.edited?"edited · ":""}${m.createdAt?.toDate?when.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):(m.unconfirmed?"not confirmed — will appear once reconnected":"sending")} · <button data-quote="${m.messageId}">Quote</button> <button data-report="${m.messageId}">Report</button>${m.participantId===state.participant?` <button data-edit="${m.messageId}">Edit</button> <button data-del="${m.messageId}">Delete</button>`:""}${isOwner&&m.participantId!==state.participant?` <button class="block-btn" data-block="${esc(m.participantId)}">Block</button>`:""}${isOwner?` <button data-pin="${m.messageId}">${room.pinnedMessageId===m.messageId?"Unpin":"Pin"}</button>`:""}</div></div></article>`;
    }
  ...
  if(name==="pricing"){
    ...
    const renderPrices=region=>{
      ...
      document.querySelector("#plans").innerHTML=
        `<button class="plan" data-plan="starter" data-region="${region}"><b>Starter</b><strong>${intl?"$1":"₦900"}/month</strong><span>Unlimited rooms · 10k messages/month · Analytics · Pins · 7-day expiry · No ads</span></button>
         <button class="plan" data-plan="pro" data-region="${region}"><b>Pro</b><strong>${intl?"$2":"₦1,500"}/month</strong><span>Everything in Starter · Custom links · 10-day expiry · Advanced analytics · Unlimited messages</span></button>`;
...
async function start(){
  ...
  // Complete a Google redirect sign-in after the page returns from Google.
  // This is intentionally background work: a failed auth request must never
  // blank the landing page or prevent the app shell from rendering.
  finishGoogleRedirect().catch(e=>{
    if(e?.code && e.code !== "auth/no-auth-event") {
      console.warn("Google redirect result unavailable:", e.code, e.message||"");
      if(route().name === "signin") {
        const msg=document.querySelector("#authmsg");
        if(msg) msg.textContent=`Google sign-in could not finish (${e.code}). You can try again without reloading WyChat.`;
      }
    }
  });
  ...
  watchAuth(async u=>{
    ...
    else if(u&&r.name==="signin")dashboard();
    else renderRoute(r);
  });
}
